...
async function posttask(task, taskdata, cookies, index) {
    const headers = {
        "Host": "edge.blocked.cc",
        "Content-Type": "application/json",
    ...
    //     .catch((err) => {
    //         console.log(index," -- post task post content：" + err);
    //     });
    // try {
    ...
    try {
        let result = await axios.post(url, taskdata, { headers: headers });
        if (result.status === 201 || result.status === 200) {
          // 请求成功
          return result.status;
        } else {
          // 请求失败，返回错误状态码
          console.log("No error but get in else", result.status)
...
// console.log(login('sei104xau42xk4gef028res0nwf8ktzlw7fxmeyc9t'))
...
async function main() {
    const fsData = fs.readFileSync("complete_sei2.txt");
    const _data = fsData
        .toString()
        .split(/[(\r\n)\r\n]+/)
        .filter((d) => d);
    const accounts = _data.map((data) => {
        ...
        return { address: info[0], dis: "A", tw: "B" };
    });
    // console.log(accounts)
    for (let i = 0; i < accounts.length; i++) {
        const account = accounts[i];
        const { address, dis, tw } = account;
        const discordId = dis!="A" ? dis : generateRandomNumberString(19);
        // console.log(discordId);
        const twname = tw!="B" ? tw : getRandomString(10);
        console.log(`Twitter:${twname} -- Discord:${discordId} -- Address:${address}`)

        let access_token = null;
        let retryCount = 0;
        while (!access_token && retryCount < 3) {
            access_token = await login(address);
            if (!access_token) {
                retryCount++;
                console.log(`${address} 登录失败，正在尝试重新登录...`);
                await sleep(1000);
            }
        }

        if (!access_token) {
            console.log(`${address} 尝试登录失败次数过多，放弃登录`);
            continue;
        }
        ...
        for (let j = 0; j < task1.length; j++) {
            ...
            const res = await posttask(task1[j], data, access_token, j);
            console.log(`${address} 任务1 -- ${j} ${res}`);
        }
        ...
        for (let j = 0; j < task2.length; j++) {
            ...
            const res = await posttask(task2[j], data, access_token, j);
        ...
        for (let j = 0; j < task3.length; j++) {
            ...
            const res = await posttask(task3[j], data, access_token, j);
        ...
        for (let j = 0; j < task4.length; j++) {
            ...
            const res = await posttask(task4[j], data, access_token, j);
            console.log(`${address} 任务4 ${res}`);
        }
        ...
        for (let j = 0; j < task6.length; j++) {
            ...
            const res = await posttask(task6[j], data, access_token, j);
            console.log(`${address} 任务6 ${res}`);
        }
    }
}
